...
{	
	...
	if(empty($cita) || empty($tipo) || empty($tipoli) || empty($hora_de_llegada) || empty($hora_de_salida) || empty($accesorio) || empty($tipo_de_locion) || empty($precio_total)) {
		...
		//updating the table
	    $result = mysqli_query($mysqli, "UPDATE inventario SET cita='$cita', tipo_de_corte='$tipo', tipo_de_limpieza='$tipoli', hora_de_llegada='$hora_de_llegada', hora_de_salida='$hora_de_salida', accesorio='$accesorio', tipo_de_locion='$tipo_de_locion', precio_total='$precio_total' WHERE id=$id");
