...
if(isset($_POST['Submit'])) {	
	...
	if(empty($cita) || empty($tipo) || empty($tipoli) || empty($hora_de_llegada) || empty($hora_de_salida) || empty($accesorio) || empty($tipo_de_locion) || empty($precio_total)) {
		...
		$result = mysqli_query($mysqli, "INSERT INTO inventario(cita, tipo_de_corte, tipo_de_limpieza, hora_de_llegada, hora_de_salida, accesorio, tipo_de_locion, precio_total, login_id) VALUES('$cita','$tipo','$tipoli','$hora_de_llegada','$hora_de_salida','$accesorio','$tipo_de_locion','$precio_total','$loginId')");
